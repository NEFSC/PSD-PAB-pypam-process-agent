...
def publish_mqtt(broker, port, topic, message):
    ...
    client = mqtt.Client(mqtt.CallbackAPIVersion.VERSION2) # Create a new MQTT client instance.
...
class NEFSCPypamAgent:
    ...
    def run(self):
        ...
        publish_mqtt("test.mosquitto.org", 1883, "PassiveAcoustics/NEFSCPypamAgency/"+self.deployment, "Initializing the pbp/pypam processing suite." )
        ...
        publish_mqtt("test.mosquitto.org", 1883, "PassiveAcoustics/NEFSCPypamAgency/"+self.deployment, "Initiating processing for audio file and netCDF generation associated with : "+ str(self.start_date) )
        logger.opt(colors=True).info("<green>running > " + command + "</green>")
        publish_mqtt("test.mosquitto.org", 1883, "PassiveAcoustics/NEFSCPypamAgency/"+self.deployment, "running > " + command )
        os.system(command)
        ...
        while self.start_date <= self.end_date:
            try:
                ...
                if not self.search_filenames(
                    ...
                    publish_mqtt("test.mosquitto.org", 1883, "PassiveAcoustics/NEFSCPypamAgency/"+self.deployment, "NetCDF file generation for "
                        + str(self.start_date)
                        + " complete!" )
                else:
                    logger.opt(colors=True).info(
                ...
                if os.name == "nt":  # For Windows-based systems
                    command = self.synth_pbp_plot_gen(
                        ...
                        cmlim=self.cmlim,
                        ylim=self.ylim,
                        nc_file=self.nc_output_dir + backslash + self.output_prefix
                        + str(self.start_date.strftime("%Y%m%d"))
                        + ".nc",
                    ...
                    command = self.synth_pbp_plot_gen(
                        ...
                        cmlim=self.cmlim,
                        ylim=self.ylim,
                        nc_file=self.nc_output_dir + backslash
                        + self.output_prefix
                        + str(self.start_date.strftime("%Y%m%d"))
                        + ".nc",
                    )  # Generate the command for plotting the NetCDF file
                ...
                if not self.search_filenames(
                    ...
                    publish_mqtt("test.mosquitto.org", 1883, "PassiveAcoustics/NEFSCPypamAgency/"+self.deployment, "Plot generation complete!" )
                ...
                logger.error(e)
                publish_mqtt("test.mosquitto.org", 1883, "PassiveAcoustics/NEFSCPypamAgency/"+self.deployment, e )
